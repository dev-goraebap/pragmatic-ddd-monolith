...
- **선택지 A — 모듈 그대로(플랫).** 컨텍스트 폴더 밑에 모듈을 평평하게 둔다. 기능 응집이 살아 있고 추가 투자 0. 애그리거트 한두 개에 자기 영역 CRUD 위주면 충분(예: `notification`).
...
| 티어 | 컨텍스트 | 구조 |
| --- | --- | --- |
| 1티어 | 단일·소수 애그리거트, 횡단 오케스트레이션 거의 없음 | A — 플랫 |
| 2티어 | 다중 애그리거트 + 횡단 오케스트레이션 잦음 | B — 레이어 |
...
| **OpenHostService + 의존 역전** | 참조를 없애지 말고 *단방향으로*. 제공 컨텍스트가 공개 입구(OHS)만 노출, 역참조가 생기면 전략 인터페이스+DIP로 끊음 | 모든 도메인을 여는 폴더 방식이 부담스러울 때 |
